time (
    ...
    RUST_BACKTRACE=1 /usr/bin/time -v target/release/taca run \
        examples/zig/explore-webgpu.opt.wasm
)
...
#     ) &&
#     time cargo build &&
#     ls -lh target/debug/taca &&
#     RUST_BACKTRACE=1 /usr/bin/time -v target/debug/taca run \
#         examples/zig/explore-simple.opt.wasm
# )
