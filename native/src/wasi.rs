...
pub fn wasi_args_get(_env: FunctionEnvMut<System>, _argv: u32, _argv_buf: u32) -> u32 {
    0
}
...
pub fn wasi_fd_close(_env: FunctionEnvMut<System>, _fd: u32) -> u32 {
    0
}
...
pub fn wasi_fd_fdstat_get(mut env: FunctionEnvMut<System>, _fd: u32, fdstat: u32) -> u32 {
    let (system, store) = env.data_and_store_mut();
    let view = system.memory.as_ref().unwrap().view(&store);
...
pub fn wasi_proc_exit(code: u32) -> std::result::Result<(), ExitCode> {
